...
public class EmailMessage {
	...
	public boolean isValid() {
		...
		int fromDot = Sender.indexOf('.');
		int toDot = Recipient.indexOf('.');
		...
		if (toat != Recipient.lastIndexOf('@')) {
			...
			/*
			Extra Part
			Implemented an improved email validation technique
			There must exist at least one '.' for the email to be considered valid
			If a dot is at start or end of email, this will also return invalid
			*/

			if (fromDot < 1 || (Sender.length() - fromDot) <= 1) {
				System.out.println("Sender address is invalid");
				return false;
			}
			if (toDot < 1 || (Recipient.length() - toDot) <= 1) {
				System.out.println("Recipient address is invalid");
				return false;
			}
